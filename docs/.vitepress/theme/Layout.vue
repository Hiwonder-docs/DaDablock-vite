...
function injectVersionSwitcher() {
  ...
  li.querySelectorAll('.version-switcher__item').forEach(item => {
    item.addEventListener('click', async () => {
      ...
      if (ver === 'latest') {
        const latestBase = location.pathname.split(`/en/${current}/`)[0] + '/en/latest/'
        window.location.href = `${latestBase}docs/index.html`
        return
      }
      const newPath = location.pathname.replace(`/en/${current}/`, `/en/${ver}/`)
      // 不同版本的文档结构可能不一致（如 latest 只有 index，其它版本有完整目录），
      // 直接沿用当前页面路径可能命中 404。这里先探测目标页面是否存在，
      // 不存在就回退到目标版本首页（根 index 会自动重定向到 /docs/index.html）。
      try {
